import styles from "./MainFunction.module.css";
...
{
    const tabsList = [
        {tabId: 'SOCIAL', displayText: 'Social',font_color:true},
        {tabId: 'GAMES', displayText: 'Games',font_color:false},
        {tabId: 'NEWS', displayText: 'News',font_color:false},
        {tabId: 'FOOD', displayText: 'Food',font_color:false},
      ];
      ...
      const defaultAppList=appsList.filter((element)=>
        {
            if(element.category==="SOCIAL")
                return element;
        })

      const [modifyAppList,setModifyAppList]=useState(defaultAppList);
      const [one,two]=useState(defaultAppList);
      const sentClickAction=(name)=>
      {
        const newList=appsList.filter((element)=>
        {
            if(element.category===name)
                return element;
        })
        setModifyAppList(newList);
        two(newList);
      }

      const [SETV,setSETV]=useState('');

      const sentChangeAction=(event)=>
      {
        let targetValue=event.target.value;
        setSETV(event);
        const searchModifyAppList=one.filter((ele)=>
        {
          return ((ele.appName).toLowerCase()).includes((targetValue.toLowerCase()))
        })
        setModifyAppList(searchModifyAppList);
      }

    return(
        <div className={styles.mainfunction_container}>
            <div className={styles.inputDiv}><input type="search" onChange={sentChangeAction} placeholder="Search"/><img src="./search.png" alt="search" height="22px"/></div>
            <TableList tabsListkey={tabsList} sentClickAction={sentClickAction} SETV={SETV}></TableList>
            <AppDisplay appsListkey={modifyAppList}></AppDisplay>
        </div>
    )
}
